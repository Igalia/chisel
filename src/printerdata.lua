...
--
function printerdata.device_id_from_ppd (ppd)
  return ppd:match ("\n%*chiselDeviceId:%s*\"([^\"]+)\"\n")
end

--- Obtains the device output mode from a PPD.
--
-- The output mode is expected to be found in the `*chiselOutputMode`
-- attribute. This will work flawlessly for PPDs generated by
-- @{printerdata:ppd}.
--
-- @param ppd String containing the PPD data.
-- @return Device output mode (string), or `nil` if not found in the input.
--
function printerdata.output_mode_from_ppd (ppd)
  return ppd:match ("\n%*chiselOutputMode:%s*\"([^\"]+)\"\n")
end
